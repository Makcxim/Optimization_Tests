...
Для получения результата необходимо запустить `cpp_gpu_algorithm.exe`

# Как закомпилить .cu в .exe

Установить NVIDIA (R) Cuda compiler driver (nvcc): https://developer.nvidia.com/cuda-downloads  
Проверить, что установка прошла успешно
```bash
nvcc --version
```
Скомпилировать `test.cu` файл в `new.exe`
```bash
nvcc -o new .\test.cu
```
На выходе получится 3 файла: `new.exe`, `new.exp`, `new.lib`  
Запускаем `new.exe` и видим вывод
...
- [x] посмотреть, в чем проблема в коде, который выполняется на видюхе
- [x] добавить гайд по запуску .cu 
- [x] написать математическое объяснение, как находятся такие числа 
- [ ] написать побольше комментов, мб перевести на англ
- [x] посчитать время выполнения на ПК - лень
